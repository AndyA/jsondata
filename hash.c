...
int
hash_start_defer( hash * h ) {
  h->defer_rehash++;
  return ERR_None;
}

int
hash_end_defer( hash * h ) {
  if ( 0 == h->defer_rehash ) {
    fprintf( stderr, "start_defer / end_defer mismatch" );
    exit( 1 );
  }
  if ( 0 == --h->defer_rehash ) {
    return _rehash( h );
...
hash_delete_key( hash * h, const void *key, size_t key_len ) {
   ...
   * However a rehash will break an iterator - so call hash_start_defer /
   * hash_end_defer around any code that deletes keys in an iterator loop.
   */
